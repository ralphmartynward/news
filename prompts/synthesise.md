...
5. If category is "event", extract event_start (ISO date YYYY-MM-DD) and optionally event_end. Dates are often in the title ("Lundi 19 mai", "Du 18 au 20 mai") or the summary. Return null for both if no date is determinable. Do not invent or guess dates.
   ...
   - umbrella days (Journées du Patrimoine, Nuit des Musées, Fête de la Musique, Nuit Blanche, etc.): these span many unrelated venues doing their own separate thing on the same date, so the bare umbrella name is not a distinct event_name — dozens of clusters would end up sharing the exact same headline. Append the specific site/venue (e.g. "Journées du Patrimoine – Capitole", "Nuit des Musées – Musée des Augustins"), or leave event_name null so the (already distinct) article title is used instead.
